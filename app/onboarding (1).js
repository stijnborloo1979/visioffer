...
"#vo-backdrop{position:fixed;inset:0;z-index:199999;background:rgba(15,23,42,.42);opacity:0;transition:opacity .35s;pointer-events:all}",
"#vo-backdrop.on{opacity:1}",

           "#vo-spotlight{position:fixed;z-index:200001;border-radius:12px;background:transparent;box-shadow:0 0 0 9999px rgba(15,23,42,.42),0 0 0 2px rgba(255,255,255,.9),0 8px 28px rgba(15,23,42,.18);transition:all .4s cubic-bezier(.4,0,.2,1);pointer-events:none}",
      "#vo-tooltip{position:fixed;z-index:200002;opacity:0;transform:translateY(12px);transition:opacity .3s,transform .3s;pointer-events:all;overflow:hidden;background:#fff;color:#0f172a;border:1px solid rgba(15,23,42,.08);border-radius:14px;box-shadow:0 18px 50px rgba(15,23,42,.22),0 4px 12px rgba(15,23,42,.08);width:340px;max-width:calc(100vw - 32px)}",
      "#vo-tooltip.on{opacity:1;transform:translateY(0)}",
      ".vo-tt-accent{height:4px;background:linear-gradient(90deg,#E8404E,#7c3aed)}",
      ".vo-tt-body{padding:22px 22px 18px}",
      ...
      ".vo-tt-text{font-size:14px;color:#475569;line-height:1.6;margin-bottom:20px}",
      ...
      ".vo-btn{padding:10px 18px;border-radius:9px;font-size:13px;font-weight:600;cursor:pointer;font-family:'Inter',sans-serif;border:none;transition:all .2s}",
      ".vo-btn-ghost{background:transparent;color:#64748b}",
      ".vo-btn-ghost:hover{color:#0f172a;background:#f1f5f9}",
      ".vo-btn-primary{background:#E8404E;color:#fff;box-shadow:0 5px 16px rgba(232,64,78,.28)}",
      ...
      "@media(max-width:480px){#vo-tooltip{width:calc(100vw - 32px);left:16px!important;right:16px!important}#vo-checklist{width:calc(100vw - 32px);right:16px;bottom:16px}.vo-tt-body{padding:20px 18px 16px}.vo-tt-title{font-size:17px}.vo-tt-text{font-size:13px}}"
    ].join("\n");
  ...
  function goStep(n) {
    ...
    _els.spotlight.style.background = "transparent";
  ...
  /* ── Checklist ───────────────────────────────────────────────────── */
  function isChecklistDismissed() { try { return localStorage.getItem(tenantKey("checklist")) === "dismissed"; } catch (e) { return false; } }
  function dismissChecklist() {
    try { localStorage.setItem(tenantKey("checklist"), "dismissed"); } catch (e) {}
    var c = document.getElementById("vo-checklist");
    if (c) { c.style.opacity = "0"; c.style.transform = "translateY(20px)"; setTimeout(function () { c.remove(); }, 300); }
  }
  ...
  function showChecklist() {
    ...
    /* Niet tonen als gebruiker heeft weggeklikt */
    ...
    cl.innerHTML =
      ...
      '<div class="vo-cl-header">' +
        '<span class="vo-cl-title" onclick="voToggleChecklist()" style="cursor:pointer;flex:1">Aan de slag</span>' +
        '<span class="vo-cl-count">' + done + '/' + TASKS.length + '</span>' +
        '<span onclick="voDismissChecklist()" style="cursor:pointer;opacity:.4;font-size:16px;margin-left:6px;line-height:1" title="Sluiten">&times;</span>' +
      '</div>' +
  ...
  global.voDismissChecklist = function () {
    dismissChecklist();
  };
