...
(function (global) {
      ...
      '<div id="sa-box">' +
        ...
        '<div id="sa-body">' +
          '<div class="sa-tabs">' +
            ...
            '<button class="sa-tab" onclick="saTab(\'integrations\',this)">Koppelingen</button>' +
          '</div>' +
          '<div id="sa-panel-list"></div>' +
          '<div id="sa-panel-add" style="display:none"></div>' +
          '<div id="sa-panel-members" style="display:none"></div>' +
          '<div id="sa-panel-integrations" style="display:none"></div>' +
        '</div>' +
      '</div>';
  ...
  global.saTab = function (id, btn) {
    ["list", "add", "members", "integrations"].forEach(function (t) {
  ...
  /* ── Tab 4: Koppelingen (cross-tenant overzicht) ──────────────── */
  async function renderIntegrations() {
    var panel = document.getElementById("sa-panel-integrations");
    if (!panel) return;
    panel.innerHTML = '<div class="sa-empty">Laden\u2026</div>';
    ...
    try {
      ...
      var intR = await c.from("qs_integrations")
        .select("id, tenant_id, type, provider, label, status, last_sync_at, last_error, product_count, created_at")
        .order("created_at", { ascending: false });
      var integrations = (intR && intR.data) || [];

      var keyR = await c.from("qs_api_keys")
        .select("id, tenant_id, key_prefix, label, scopes, integration_id, last_used_at, revoked_at, created_at")
        .is("revoked_at", null)
        .order("created_at", { ascending: false });
      var apiKeys = (keyR && keyR.data) || [];

      if (!_tenants.length) await loadTenants();
      var tenantMap = {};
      _tenants.forEach(function (t) { tenantMap[t.slug || t.id] = t.company_name || t.slug; });

      // Add button always visible
      var addBtnHtml = '<button class="sa-btn" onclick="saAddIntegration()" style="margin-bottom:14px;display:flex;align-items:center;gap:6px">' +
        '<span style="font-size:15px">+</span> Koppeling toevoegen voor klant</button>';

      if (!integrations.length && !apiKeys.length) {
        panel.innerHTML = addBtnHtml + '<div class="sa-empty">Nog geen koppelingen bij klanten.</div>';
        return;
      }

      var activeCount = integrations.filter(function (i) { return i.status === "active"; }).length;
      var errorCount = integrations.filter(function (i) { return i.status === "error"; }).length;
      var keyCount = apiKeys.length;

      var html = addBtnHtml +
      '<div style="display:flex;gap:10px;margin-bottom:16px">' +
        '<div style="flex:1;padding:10px 14px;background:#f0fdf4;border-radius:8px;text-align:center">' +
          '<div style="font-size:20px;font-weight:700;color:#166534">' + activeCount + '</div>' +
          '<div style="font-size:11px;color:#15803d">Actief</div>' +
        '</div>' +
        '<div style="flex:1;padding:10px 14px;background:' + (errorCount ? '#fef2f2' : '#f9fafb') + ';border-radius:8px;text-align:center">' +
          '<div style="font-size:20px;font-weight:700;color:' + (errorCount ? '#991b1b' : '#6b7280') + '">' + errorCount + '</div>' +
          '<div style="font-size:11px;color:' + (errorCount ? '#dc2626' : '#9ca3af') + '">Fouten</div>' +
        '</div>' +
        '<div style="flex:1;padding:10px 14px;background:#eff6ff;border-radius:8px;text-align:center">' +
          '<div style="font-size:20px;font-weight:700;color:#1e40af">' + keyCount + '</div>' +
          '<div style="font-size:11px;color:#2563eb">API-sleutels</div>' +
        '</div>' +
      '</div>';

      var byTenant = {};
      integrations.forEach(function (i) {
        if (!byTenant[i.tenant_id]) byTenant[i.tenant_id] = [];
        byTenant[i.tenant_id].push(i);
      });
      apiKeys.forEach(function (k) {
        if (!byTenant[k.tenant_id]) byTenant[k.tenant_id] = [];
      });

      var typeLabels = { supplier: "Leverancier", accounting: "Boekhouding", crm: "CRM", custom: "Custom" };
      var statusDots = { active: "#10b981", setup: "#3b82f6", error: "#ef4444", paused: "#9ca3af" };
      var statusLabels = { active: "Actief", setup: "Instellen", error: "Fout", paused: "Gepauzeerd" };

      Object.keys(byTenant).sort().forEach(function (tid) {
        var tName = tenantMap[tid] || tid;
        var tIntg = byTenant[tid];
        var tKeys = apiKeys.filter(function (k) { return k.tenant_id === tid; });

        html += '<div class="sa-card">' +
          '<div style="font-size:13px;font-weight:700;color:#1e293b;margin-bottom:8px">' +
            '<span class="sa-badge" style="background:#eef2ff;color:#3730a3;margin-right:6px">' + esc(tid) + '</span>' +
            esc(tName) +
          '</div>';

        tIntg.forEach(function (intg) {
          var dotClr = statusDots[intg.status] || "#9ca3af";
          var stText = statusLabels[intg.status] || intg.status;
          var syncText = intg.last_sync_at ? saTimeAgo(intg.last_sync_at) : "nooit";

          html += '<div style="display:flex;align-items:center;gap:8px;padding:6px 0;border-top:1px solid #f3f4f6;font-size:12px">' +
            '<span style="width:7px;height:7px;border-radius:50%;background:' + dotClr + ';flex-shrink:0"></span>' +
            '<span style="font-weight:600;color:#374151;flex:1">' + esc(intg.label) + '</span>' +
            '<span style="color:#9ca3af">' + esc(typeLabels[intg.type] || intg.type) + ' \u00b7 ' + esc(intg.provider) + '</span>' +
            '<span style="color:#9ca3af">sync: ' + syncText + '</span>' +
            '<span style="font-size:10px;padding:2px 8px;border-radius:4px;' +
              'background:' + (intg.status === "active" ? "#ecfdf5" : intg.status === "error" ? "#fef2f2" : "#f3f4f6") + ';' +
              'color:' + dotClr + ';font-weight:600">' + esc(stText) + '</span>' +
          '</div>';

          if (intg.last_error) {
            html += '<div style="font-size:11px;color:#ef4444;padding:2px 0 4px 15px">\u26a0 ' + esc(intg.last_error) + '</div>';
          }
        });
        ...
        if (tKeys.length) {
          ...
          tKeys.forEach(function (k) {
            html += '<div style="display:flex;align-items:center;gap:8px;padding:3px 0;font-size:11px;color:#6b7280">' +
              '\ud83d\udd11 <span style="font-weight:600;color:#374151">' + esc(k.label) + '</span>' +
              ' <code style="font-size:10px;color:#9ca3af">' + esc(k.key_prefix) + '\u2026</code>' +
              ' <span style="color:#9ca3af">' + (k.last_used_at ? 'laatst: ' + saTimeAgo(k.last_used_at) : 'nooit gebruikt') + '</span>' +
            '</div>';
          });
          html += '</div>';
        }
  ...
  /* ── Koppeling aanmaken vanuit super-admin ─────────────────────── */
  global.saAddIntegration = async function () {
    if (!_tenants.length) await loadTenants();
    var activeTenants = _tenants.filter(function (t) { return t.slug !== "default"; });

    var overlay = document.createElement("div");
    overlay.style.cssText = "position:fixed;inset:0;z-index:200000;background:rgba(15,23,42,.55);display:flex;align-items:center;justify-content:center;padding:16px";
    overlay.addEventListener("click", function (e) { if (e.target === overlay) overlay.remove(); });

    var providers = [
      { id: "custom_api", label: "Eigen API / ERP",    type: "supplier" },
      { id: "csv_upload", label: "CSV / Excel upload",  type: "supplier" },
      { id: "techdata",   label: "TD Synnex",           type: "supplier" },
      { id: "ingram",     label: "Ingram Micro",        type: "supplier" },
      { id: "also",       label: "ALSO",                type: "supplier" },
    ];

    var tenantOpts = activeTenants.map(function (t) {
    ...
    box.style.cssText = "background:#fff;border-radius:14px;padding:24px;width:380px;max-width:90vw;font-family:Inter,Arial,sans-serif";
    box.innerHTML =
      '<h3 style="margin:0 0 16px;font-size:17px;font-weight:700;color:#1e293b">Koppeling toevoegen</h3>' +
      '<label style="font-size:12px;font-weight:600;color:#4b5563;display:block;margin-bottom:4px">Klant</label>' +
      '<select id="sa-intg-tenant" style="width:100%;padding:8px 10px;border:1px solid #d1d5db;border-radius:8px;font-size:13px;margin-bottom:12px;font-family:inherit">' +
        '<option value="">— Kies een klant —</option>' + tenantOpts +
      '</select>' +
      '<label style="font-size:12px;font-weight:600;color:#4b5563;display:block;margin-bottom:4px">Provider</label>' +
      '<select id="sa-intg-prov" style="width:100%;padding:8px 10px;border:1px solid #d1d5db;border-radius:8px;font-size:13px;margin-bottom:12px;font-family:inherit">' +
        provOpts +
      '</select>' +
      '<label style="font-size:12px;font-weight:600;color:#4b5563;display:block;margin-bottom:4px">Naam</label>' +
      '<input id="sa-intg-label" type="text" placeholder="bv. Techdata prijslijst" ' +
        'style="width:100%;padding:8px 10px;border:1px solid #d1d5db;border-radius:8px;font-size:13px;margin-bottom:12px;font-family:inherit;box-sizing:border-box">' +
      '<div id="sa-intg-err" style="font-size:12px;color:#dc2626;min-height:18px;margin-bottom:8px"></div>' +
      '<div style="display:flex;gap:8px;justify-content:flex-end">' +
        '<button class="sa-btn sa-btn-s" onclick="this.closest(\'div\').closest(\'div\').closest(\'div\').remove()" style="color:#6b7280">Annuleren</button>' +
        '<button class="sa-btn sa-btn-s" id="sa-intg-save" style="background:#E8404E;color:#fff;border-color:#E8404E;font-weight:600">Aanmaken + API-sleutel</button>' +
      '</div>';

    overlay.appendChild(box);
    document.body.appendChild(overlay);

    document.getElementById("sa-intg-save").addEventListener("click", async function () {
      var tenant = document.getElementById("sa-intg-tenant").value;
      var provider = document.getElementById("sa-intg-prov").value;
      var label = document.getElementById("sa-intg-label").value.trim();
      var errEl = document.getElementById("sa-intg-err");

      if (!tenant) { errEl.textContent = "Kies een klant"; return; }
      if (!label) { label = providers.find(function (p) { return p.id === provider; }).label; }

      errEl.textContent = "Bezig…";
      this.disabled = true;

      try {
        var intgRes = await workerCall("create-integration", {
          tenant_id: tenant,
          type: "supplier",
          provider: provider,
          label: label,
        ...
        var keyRes = await workerCall("create-api-key", {
          tenant_id: tenant,
          label: label,
          integration_id: intgRes.integration ? intgRes.integration.id : null,
        });

        overlay.remove();

        // Show key in a result dialog
        if (keyRes.key) {
          saShowKey(keyRes.key, label, tenant);
        }

        renderIntegrations();
      } catch (e) {
        errEl.textContent = e.message || "Aanmaken mislukt";
        if (e.message) console.error("create-integration fout:", e);
        this.disabled = false;
      }
    });
  };

  function saShowKey(key, label, tenant) {
    var overlay = document.createElement("div");
    overlay.style.cssText = "position:fixed;inset:0;z-index:200001;background:rgba(15,23,42,.55);display:flex;align-items:center;justify-content:center;padding:16px";

    var box = document.createElement("div");
    box.style.cssText = "background:#fff;border-radius:14px;padding:24px;width:420px;max-width:90vw;font-family:Inter,Arial,sans-serif";
    box.innerHTML =
      '<div style="text-align:center;font-size:28px;margin-bottom:8px">\ud83d\udd11</div>' +
      '<h3 style="margin:0 0 4px;font-size:17px;font-weight:700;text-align:center">API-sleutel aangemaakt</h3>' +
      '<p style="font-size:12px;color:#9ca3af;text-align:center;margin:0 0 14px">Tenant: <strong>' + esc(tenant) + '</strong> \u00b7 ' + esc(label) + '</p>' +
      '<div style="display:flex;align-items:center;gap:8px;background:#f9fafb;border:1px solid #e5e7eb;border-radius:8px;padding:10px 12px;margin-bottom:14px">' +
        '<code id="sa-key-val" style="flex:1;font-size:11px;word-break:break-all;line-height:1.4;color:#1f2937">' + esc(key) + '</code>' +
        '<button class="sa-btn sa-btn-s" id="sa-key-copy" style="background:#E8404E;color:#fff;border-color:#E8404E;flex-shrink:0">Kopiëren</button>' +
      '</div>' +
      '<pre style="background:#1f2937;color:#e5e7eb;border-radius:8px;padding:12px;font-size:10px;line-height:1.5;overflow:auto;margin:0 0 14px;font-family:Consolas,monospace">' +
        'curl -X POST ' + workerUrl() + '/api/v1/products \\\n' +
        '  -H "Authorization: Bearer ' + esc(key.slice(0, 14)) + '..." \\\n' +
        '  -H "Content-Type: application/json" \\\n' +
        '  -d \'{"action":"upsert","products":[\n    {"ref":"ART-001","name":"Test","price":99}\n  ]}\'' +
      '</pre>' +
      '<button class="sa-btn" onclick="this.closest(\'div\').closest(\'div\').remove()" style="width:100%;text-align:center">Sluiten</button>';

    overlay.appendChild(box);
    document.body.appendChild(overlay);

    document.getElementById("sa-key-copy").addEventListener("click", function () {
      var val = document.getElementById("sa-key-val").textContent;
      navigator.clipboard.writeText(val).then(function () {
        document.getElementById("sa-key-copy").textContent = "\u2713 Gekopieerd";
        setTimeout(function () { document.getElementById("sa-key-copy").textContent = "Kopiëren"; }, 2000);
      });
    });
  }

  function saTimeAgo(iso) {
    if (!iso) return "nooit";
