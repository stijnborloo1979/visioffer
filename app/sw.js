...
self.addEventListener("fetch", function(e) {
  ...
  // Alleen GET komt door de SW. POST/PUT/DELETE gaan direct naar het netwerk.
  ...
  if(req.mode === "navigate") {
    ...
    if(hasOwnPage || !isAppRoot) {
      ...
      e.respondWith(
        fetch(req).then(function(r){
          if(r && r.status === 200){
            var cl = r.clone();
            caches.open(CACHE).then(function(c){ return c.put(req, cl); })
              .catch(function(err){ console.log("SW put skip:", err && err.message); });
          }
          return r;
    ...
    // Kale app-navigatie: verse index.html (network-first, cache-fallback)
    e.respondWith(
      fetch(INDEX, { cache: "no-cache" }).then(function(r) {
        if(r && r.status === 200) {
          var clone = r.clone();
          caches.open(CACHE).then(function(c) { return c.put(INDEX, clone); })
            .catch(function(err){ console.log("SW put skip:", err && err.message); });
        }
        return r;
      }).catch(function() {
  ...
  e.respondWith(
    fetch(req).then(function(r) {
      if(r && r.status === 200) {
        var clone = r.clone();
        caches.open(CACHE).then(function(c) { return c.put(req, clone); })
          .catch(function(err){ console.log("SW cache put skip:", err && err.message); });
      }
      return r;
    }).catch(function() {
