/* ═══════════════════════════════════════════════════════════════════════
   ...
   Integrations & API (POST, JSON):
     /api/v1/products      EXTERN — API-key auth (viso_live_...), product CRUD
     /list-integrations    admin: koppelingen oplijsten
     /create-integration   admin: koppeling aanmaken
     /update-integration   admin: koppeling bijwerken
     /delete-integration   admin: koppeling verwijderen
     /list-api-keys        admin: API-sleutels oplijsten
     /create-api-key       admin: nieuwe API-sleutel genereren
     /revoke-api-key       admin: sleutel intrekken
     /list-sync-log        admin: sync-logboek bekijken
   ═══════════════════════════════════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════════════════════════════════
   INTEGRATIONS — helpers voor API-key auth en product sync
   ═══════════════════════════════════════════════════════════════════════ */
...
function generateApiKey() {
  ...
  const raw = [...bytes]
    ...
    .join("")
    .slice(0, 32);
  return "viso_live_" + raw;
}

/* API-key authenticatie (Bearer viso_live_...) */
async function getApiCaller(env, request) {
  const auth = request.headers.get("Authorization") || "";
  const token = auth.replace(/^Bearer\s+/i, "").trim();
  if (!token || !token.startsWith("viso_live_")) return null;

  const hash = await sha256(token);
  const r = await sbAdmin(env, "/rest/v1/rpc/lookup_api_key", {
    method: "POST",
  ...
  sbAdmin(env, "/rest/v1/qs_api_keys?id=eq." + key.key_id, {
    method: "PATCH",
    headers: { Prefer: "return=minimal" },
...
/* Supabase REST query met tenant filter */
async function sbRest(env, table, query, opts = {}) {
  return sbAdmin(env, "/rest/v1/" + table + "?" + query, opts);
}

/* Sync log entry */
async function logSync(env, caller, action, count, status, error, ip) {
  await sbAdmin(env, "/rest/v1/qs_api_sync_log", {
    method: "POST",
    headers: { Prefer: "return=minimal" },
    body: JSON.stringify({
      tenant_id: caller.tenant,
      api_key_id: caller.keyId,
      integration_id: caller.integrationId,
      action,
      item_count: count,
      status,
...
/* ── Product API handler (API-key auth) ───────────────────────────────── */
async function handleApiProducts(env, request, caller, ip) {
  let body = {};
  try {
  ...
  const tenantFilter =
    "tenant_id=eq." + encodeURIComponent(caller.tenant);

  /* LIST */
  if (action === "list") {
    if (!caller.scopes.includes("products.read"))
      return json({ error: "Scope products.read ontbreekt" }, 403);

    let q =
      tenantFilter +
      "&select=id,name,ref,description,category,vendor,supplier,cost,price,margin,image_url,external_ref,supplier_id,last_synced_at,created_at&order=name.asc";
    if (body.supplier_id)
      q += "&supplier_id=eq." + encodeURIComponent(body.supplier_id);
    if (body.category)
      q += "&category=eq." + encodeURIComponent(body.category);
    if (body.limit)
      q += "&limit=" + Math.min(Number(body.limit) || 100, 1000);
    if (body.offset) q += "&offset=" + (Number(body.offset) || 0);

    const r = await sbRest(env, "qs_products", q);
    await logSync(
      env, caller, "products.list",
      (r.body || []).length, "ok", null, ip
    );
    return json({ products: r.body || [] });
  }

  /* UPSERT (bulk) */
  if (action === "upsert") {
    if (!caller.scopes.includes("products.write"))
      return json({ error: "Scope products.write ontbreekt" }, 403);

    const items = Array.isArray(body.products) ? body.products : [];
    if (!items.length) return json({ error: "products[] is leeg" }, 400);
    if (items.length > 500)
      return json({ error: "Max 500 producten per batch" }, 400);

    const now = new Date().toISOString();
    const rows = items.map((p) => ({
      tenant_id: caller.tenant,
      name: String(p.name || "").slice(0, 200),
      ref: p.ref ? String(p.ref).slice(0, 80) : null,
      description: p.description
        ? String(p.description).slice(0, 2000)
        : null,
      category: p.category ? String(p.category).slice(0, 100) : null,
      vendor: p.vendor ? String(p.vendor).slice(0, 100) : null,
      supplier: p.supplier ? String(p.supplier).slice(0, 100) : null,
      ...
      external_ref: p.external_ref
        ? String(p.external_ref).slice(0, 100)
        : p.ref
          ? String(p.ref).slice(0, 100)
          : null,
      supplier_id: caller.integrationId || null,
      last_synced_at: now,
    }));

    const ins = await sbAdmin(env, "/rest/v1/qs_products", {
      method: "POST",
      headers: {
    ...
    if (!ins.ok) {
      await logSync(
        env, caller, "products.upsert",
        items.length, "error", JSON.stringify(ins.body), ip
      );
      return json({ error: "Upsert mislukt", detail: ins.body }, 500);
    }

    if (caller.integrationId) {
      sbRest(
        env, "qs_integrations",
        "id=eq." + encodeURIComponent(caller.integrationId),
        {
          method: "PATCH",
          headers: { Prefer: "return=minimal" },
          body: JSON.stringify({
    ...
    await logSync(
      env, caller, "products.upsert",
      rows.length, "ok", null, ip
    );
    return json({ ok: true, upserted: (ins.body || []).length });
  }
  ...
  if (action === "delete") {
    ...
    const refs = Array.isArray(body.external_refs)
      ? body.external_refs
      : [];
    const ids = Array.isArray(body.ids) ? body.ids : [];
    if (!refs.length && !ids.length)
      return json({ error: "external_refs[] of ids[] vereist" }, 400);
    ...
    if (refs.length) {
      const del = await sbRest(
        ...
        tenantFilter +
          "&external_ref=in.(" +
          refs.map(encodeURIComponent).join(",") +
          ")",
        { method: "DELETE", headers: { Prefer: "return=representation" } }
      );
      deleted += (del.body || []).length;
    }
    if (ids.length) {
      const del = await sbRest(
        env, "qs_products",
...
const handlers = {
  ...
  /* ── Integrations — koppelingenbeheer (JWT auth, admin) ────────────── */

  async "list-integrations"(env, body, caller) {
    if (!caller || !caller.isAdmin) return json({ error: "Alleen voor beheerders" }, 403);
    const r = await sbRest(env, "qs_integrations",
      "tenant_id=eq." + encodeURIComponent(caller.tenant) +
      "&select=*&order=created_at.desc"
    );
    return json({ integrations: r.body || [] });
  },

  async "create-integration"(env, body, caller) {
    if (!caller || !caller.isAdmin) return json({ error: "Alleen voor beheerders" }, 403);
    const type = String(body.type || "supplier");
    if (!["supplier", "accounting", "crm", "custom"].includes(type))
      return json({ error: "Ongeldig type" }, 400);

    let tenant = caller.tenant;
    if (caller.isSuper && body.tenant_id) tenant = String(body.tenant_id);
    if (!tenant) return json({ error: "tenant_id ontbreekt" }, 400);

    const row = {
      tenant_id: tenant,
      type,
      provider: String(body.provider || "custom_api").slice(0, 60),
      label: String(body.label || "Nieuwe koppeling").slice(0, 120),
      config: body.config || {},
      sync_direction: body.sync_direction || "inbound",
    };

    const ins = await sbAdmin(env, "/rest/v1/qs_integrations", {
      method: "POST",
      headers: { Prefer: "return=representation" },
      body: JSON.stringify(row),
    });
    if (!ins.ok) return json({ error: "Aanmaken mislukt" }, 500);
    return json({ ok: true, integration: ins.body[0] });
  },

  async "update-integration"(env, body, caller) {
    if (!caller || !caller.isAdmin) return json({ error: "Alleen voor beheerders" }, 403);
    const id = body.id;
    ...
    if (!caller.isSuper) q += "&tenant_id=eq." + encodeURIComponent(caller.tenant);

    await sbRest(env, "qs_integrations", q, {
      method: "PATCH",
      headers: { Prefer: "return=representation" },
  ...
  async "delete-integration"(env, body, caller) {
    if (!caller || !caller.isAdmin) return json({ error: "Alleen voor beheerders" }, 403);
    const id = body.id;
    if (!id) return json({ error: "id ontbreekt" }, 400);
    ...
    await sbRest(env, "qs_integrations", q, { method: "DELETE" });
    return json({ ok: true });
  },

  /* ── API keys — sleutelbeheer (JWT auth, admin) ──────────────────── */

  async "list-api-keys"(env, body, caller) {
    if (!caller || !caller.isAdmin) return json({ error: "Alleen voor beheerders" }, 403);
    const r = await sbRest(env, "qs_api_keys",
      "tenant_id=eq." + encodeURIComponent(caller.tenant) +
      "&select=id,key_prefix,label,scopes,integration_id,last_used_at,revoked_at,created_at" +
      "&order=created_at.desc"
    );
    return json({ keys: r.body || [] });
  },

  async "create-api-key"(env, body, caller) {
    if (!caller || !caller.isAdmin) return json({ error: "Alleen voor beheerders" }, 403);
    const plainKey = generateApiKey();
    const hash = await sha256(plainKey);
    const prefix = plainKey.slice(0, 14);
    const label = String(body.label || "API-sleutel").slice(0, 80);
    ...
    const ins = await sbAdmin(env, "/rest/v1/qs_api_keys", {
      method: "POST",
      headers: { Prefer: "return=representation" },
      body: JSON.stringify({
        tenant_id: tenant,
    ...
    return json({
      ...
      prefix,
      label,
      warning: "Bewaar deze sleutel veilig — hij wordt niet meer getoond.",
    });
  },

  async "revoke-api-key"(env, body, caller) {
    if (!caller || !caller.isAdmin) return json({ error: "Alleen voor beheerders" }, 403);
    const id = body.key_id;
    if (!id) return json({ error: "key_id ontbreekt" }, 400);

    let q = "id=eq." + encodeURIComponent(id);
    if (!caller.isSuper) q += "&tenant_id=eq." + encodeURIComponent(caller.tenant);

    await sbRest(env, "qs_api_keys", q, {
      method: "PATCH",
      headers: { Prefer: "return=minimal" },
  ...
  async "list-sync-log"(env, body, caller) {
    if (!caller || !caller.isAdmin) return json({ error: "Alleen voor beheerders" }, 403);
    const limit = Math.min(body.limit || 50, 200);
    let q = "tenant_id=eq." + encodeURIComponent(caller.tenant) +
      "&select=*&order=created_at.desc&limit=" + limit;
    if (body.integration_id)
      q += "&integration_id=eq." + encodeURIComponent(body.integration_id);

    const r = await sbRest(env, "qs_api_sync_log", q);
    return json({ log: r.body || [] });
  },
...
export default {
  async fetch(request, env) {
    ...
    /* ── Public API endpoints (API-key auth: viso_live_...) ───────────── */
    if (path.startsWith("api/v1/")) {
      const apiCaller = await getApiCaller(env, request);
      if (!apiCaller)
      ...
      if (sub === "products") {
        return await handleApiProducts(env, request, apiCaller, ip);
      }
      return json({ error: "Onbekend API-endpoint: /" + path }, 404);
    }
